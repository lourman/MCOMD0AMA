...
### Integer Numbers (Z)
Another set of numbers are all whole numbers including negative whole numbers. The range of integer numbers are from negative infinity to zero and from zero to positive infinity.

```{r, out.width='60%', fig.align='center', fig.cap='Integer Numbers', echo=FALSE}
knitr::include_graphics('images/Integer Numbers.png')
```
Integers are a commonly used data type in computer programming. For example, whenever a number is being incremented, 9isuch as within a "for loop" or "while loop," an integer is used. Integers are also used to determine an item's location within an array.
...
### Rational Numbers (Q)
All numbers that can be expressed as a fraction (ratio) are called rational numbers, as long as the denominator is not zero. 
